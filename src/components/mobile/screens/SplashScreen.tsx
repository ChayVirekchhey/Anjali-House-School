...
import { CheckCircle2, ShieldCheck, Zap, ArrowRight, School, Heart, Moon, Sun, UserPlus, LogIn, Sparkles } from 'lucide-react';
...
export const SplashScreen: React.FC<SplashScreenProps> = ({ onContinue, isMockup = false }) => {
  ...
  const handleStartDirect = (role: 'teacher' | 'student' | 'admin') => {
    loginAsGuest(role === 'admin' ? 'admin' : role);
    if (role === 'student') {
  ...
  return (
    <div className="relative w-full h-full min-h-[580px] bg-gradient-to-b from-[#004D35] via-[#006B45] to-[#003826] text-white flex flex-col justify-between p-6 overflow-hidden select-none">
      ...
      {/* Bottom Action Area with Get Started & Login/Register options */}
      <div className="relative z-10 space-y-2.5 pb-2 w-full max-w-xs mx-auto">
        {/* Primary Action: GET STARTED (Direct instant access without login or registration) */}
        <button
        ...
        <p className="text-center text-[10px] text-emerald-200/80 font-medium">
          {t.noAccountNeeded}
        </p>

        {/* Secondary Actions: Sign In & Register buttons for registered users */}
        <div className="grid grid-cols-2 gap-2 pt-1">
          <button
            onClick={() => {
              if (isMockup && onContinue) onContinue();
              else handleGoToAuth('login');
            }}
            className="py-2.5 px-3 rounded-xl bg-white/15 hover:bg-white/25 border border-white/20 text-white font-bold text-xs backdrop-blur-xs transition-all active:scale-[0.98] flex items-center justify-center gap-1.5 cursor-pointer"
          >
        ...
        {!isMockup && (
          <div className="pt-1 flex items-center justify-center gap-4 text-xs text-emerald-200/80">
            <button 
